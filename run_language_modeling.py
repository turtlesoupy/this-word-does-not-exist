...
def collate_wiki(tokenizer, examples: List[Tuple[torch.Tensor, torch.Tensor]]):
    ...
    if tokenizer._pad_token is None:
        return (pad_sequence([e[0] for e in examples], batch_first=True), padded_bool_mask)
    return (pad_sequence([e[0] for e in examples], batch_first=True, padding_value=tokenizer.pad_token_id), padded_bool_mask)
