...
def collate_wiki(tokenizer, examples: List[Tuple[torch.Tensor, torch.Tensor]]):
    padded_bool_mask = pad_sequence(
        [e[1] for e in examples], batch_first=True, padding_value=False
    )
    if tokenizer._pad_token is None:
        return (
...
def load_and_cache_examples(args, tokenizer, evaluate=False):
    ...
    if args.line_by_line:
        ...
        return datasets.WikiArticleTitleDataset(
            tokenizer, args, file_path=file_path, block_size=args.block_size
        )
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Whether this is a parsed dictionary dataset",
    )
    parser.add_argument(
        "--urban_dictionary_dataset",
        action="store_true",
        help="Whether this is an urban dictionary dataset",
    )
    parser.add_argument(
        "--eval_subsampling",
        type=float,
        default=1.0,
        help="Amount of sub-sampling of evaluation set",
    )
    ...
    parser.add_argument(
        ...
        default=1.0,
        type=float,
        help="How much to scale up title for wiki/dictionary dataset tokens",
    )
    parser.add_argument(
        "--should_continue",
        action="store_true",
    ...
    parser.add_argument(
        "--summary_comment", type=str, help="Summary comment for tensorboard"
    )
    ...
    if args.urban_dictionary_dataset or args.parsed_dictionary_dataset:
        logger.info(
            "Urban dictionary dataset: adding special tokens and resizing model"
        )
